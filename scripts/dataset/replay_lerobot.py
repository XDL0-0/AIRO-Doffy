...
Usage:
  ...
  python -m scripts.dataset.replay_lerobot \
...
def main() -> None:
    ...
    parser.add_argument(
        "--confirm-hardware",
        action="store_true",
        help="Acknowledge physical robot motion. Required unless --no_robot is set.",
    )
    ...
    if not args.no_robot and not args.confirm_hardware:
        parser.error("Refusing robot access without --confirm-hardware (or use --no_robot).")
    ...
    def move_to_initial_pose(pose: np.ndarray):
        nonlocal delta_tcp_target_pose
