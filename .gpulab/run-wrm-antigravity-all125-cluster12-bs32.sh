...
stage_dir="/tmp/airo_wrm_antigravity_all125_cluster12_stage"
source_archive="${stage_dir}/airo-doffy-wrm-antigravity-all125-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-antigravity-all125-src.sha256"
...
work_root="/tmp/airo_wrm_antigravity_all125_cluster12_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_antigravity_all125_cluster12_bs32"
policy_output="${output_root}/WRM_antigravity_all_train"
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed all-125 WRM_antigravity policy found; skipping"
else
    echo "[$(date -Is)] starting all-125 WRM_antigravity on cluster 12"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_antigravity_all_train.yaml \
        --wandb-project AIRO-Doffy-WRM-Grasp \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_antigravity-all125-cluster12-bs32" \
...
milestones = sorted(folder.glob("WRM_antigravity_step_*.pt"))
last = folder / "last.pt"
...
if checkpoint.get("kind") != "WRM_antigravity":
...
echo "[$(date -Is)] completed all-125 WRM_antigravity training"
