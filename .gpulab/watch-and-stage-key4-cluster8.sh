...
source_archive="/tmp/airo-doffy-key4-src.tar.gz"
dataset_archive="/tmp/WRM_grasp_cylinder_different_sizes_lero.tar"
runner_script="/home/yuyuan/AIRO-Doffy/.gpulab/run-key4-sequential-cluster8.sh"
...
while true; do
    ...
    echo "[$(date -Is)] cluster8 Key4 ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
for ssh_attempt in $(seq 1 20); do
    if ssh \
        ...
        -o ConnectTimeout=20 \
        "${ssh_alias}" \
        'mkdir -p /tmp/airo_key4_stage && chmod 755 /tmp/airo_key4_stage'; then
        break
    fi
...
scp -o BatchMode=yes \
    ...
    "${dataset_archive}" \
    "${runner_script}" \
    "${ssh_alias}:/tmp/airo_key4_stage/"
scp -o BatchMode=yes \
    "${hf_token_file}" \
    "${ssh_alias}:/tmp/airo_key4_stage/hf_token"
scp -o BatchMode=yes \
    "${wandb_netrc_file}" \
    "${ssh_alias}:/tmp/airo_key4_stage/netrc"

ssh -o BatchMode=yes "${ssh_alias}" \
    "chown -R 1000:100 /tmp/airo_key4_stage; \
    chmod 755 /tmp/airo_key4_stage \
        /tmp/airo_key4_stage/run-key4-sequential-cluster8.sh; \
    chmod 600 /tmp/airo_key4_stage/hf_token \
        /tmp/airo_key4_stage/netrc; \
    echo '${source_sha256}  /tmp/airo_key4_stage/airo-doffy-key4-src.tar.gz' \
        | sha256sum --check --strict; \
    echo '${dataset_sha256}  /tmp/airo_key4_stage/WRM_grasp_cylinder_different_sizes_lero.tar' \
        | sha256sum --check --strict; \
    touch /tmp/airo_key4_stage/READY; \
    chown 1000:100 /tmp/airo_key4_stage/READY"

echo "Staged inputs and released cluster8 Key4 job ${job_id}"
