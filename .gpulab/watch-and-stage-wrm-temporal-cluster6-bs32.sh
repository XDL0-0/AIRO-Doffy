...
source_archive="/tmp/airo-doffy-wrm-temporal-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-temporal-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-temporal-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_temporal_cluster6_stage"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 WRM_temporal ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-temporal-cluster6-bs32.sh; \
...
echo "Staged local source/dataset archives and released Cluster6 WRM_temporal job ${job_id}"
