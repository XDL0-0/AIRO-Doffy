...
stage_dir="/tmp/airo_wrm_qwen_all125_cluster6_stage"
source_archive="${stage_dir}/airo-doffy-wrm-qwen-all125-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-qwen-all125-src.sha256"
...
work_root="/tmp/airo_wrm_qwen_all125_cluster6_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_qwen_all125_cluster6_bs32"
policy_output="${output_root}/WRM_qwen_all_train"
policy_log="${policy_output}/cluster6_bs32_train.log"
wandb_project="AIRO-Doffy-WRM-Grasp"
batch_size=32
num_workers=8
upload_hf="${UPLOAD_HF:-0}"
hf_repo_id="${HF_REPO_ID:-IXDLI/AIRO-Doffy-WRM-Grasp-WRM-qwen-all125}"
...
tests = sorted(Path("policies/realman_beaver/tests").glob("test_*qwen*.py"))
if not tests:
    raise SystemExit("No focused WRM_qwen unit-test module is present in the source archive")
print("qwen_test_modules", ",".join(str(path) for path in tests))
PY
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed WRM_qwen all-125 policy found; skipping training"
else
    echo "[$(date -Is)] starting WRM_qwen all-125 final training; batch_size=${batch_size}; num_workers=${num_workers}"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_qwen_all_train.yaml \
        --wandb-project "${wandb_project}" \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_qwen-all125-cluster6-bs32" \
...
milestones = sorted(folder.glob("WRM_qwen_step_*.pt"))
if len(milestones) != 10 or not (folder / "last.pt").is_file():
    raise SystemExit(
        "Refusing incomplete WRM_qwen all-125 upload: "
...
api.upload_folder(
    ...
    commit_message="Upload WRM_qwen all-125 checkpoints through step 100000",
)
...
echo "[$(date -Is)] completed Cluster6 batch-size-32 WRM_qwen all-125 training"
