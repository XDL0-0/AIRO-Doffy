...
source_archive="/tmp/airo-doffy-wrm-wrap-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-wrap-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-wrap-stratified-cluster9-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_wrap_stratified_cluster9_stage"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster9 WRM_wrap ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-wrap-stratified-cluster9-bs32.sh; \
...
echo "Staged local source/dataset archives and released Cluster9 WRM_wrap job ${job_id}"
