...
source_archive="/tmp/airo-doffy-wrm-qwen-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-qwen-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-qwen-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_qwen_cluster6_stage"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 WRM_qwen ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-qwen-cluster6-bs32.sh; \
...
echo "Staged local source/dataset archives and released Cluster6 WRM_qwen job ${job_id}"
