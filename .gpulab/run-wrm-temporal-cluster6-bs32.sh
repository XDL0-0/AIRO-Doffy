...
stage_dir="/tmp/airo_wrm_temporal_cluster6_stage"
source_archive="${stage_dir}/airo-doffy-wrm-temporal-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-temporal-src.sha256"
...
work_root="/tmp/airo_wrm_temporal_cluster6_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_temporal_cluster6_bs32"
policy_output="${output_root}/WRM_temporal"
policy_log="${policy_output}/cluster6_bs32_train.log"
wandb_project="AIRO-Doffy-WRM-Grasp"
batch_size=32
num_workers=8
upload_hf="${UPLOAD_HF:-0}"
hf_repo_id="${HF_REPO_ID:-IXDLI/AIRO-Doffy-WRM-Grasp-WRM-temporal}"
...
tests = sorted(Path("policies/realman_beaver/tests").glob("test_*temporal*.py"))
if not tests:
    raise SystemExit("No focused temporal unit-test module is present in the source archive")
print("temporal_test_modules", ",".join(str(path) for path in tests))
PY
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed WRM_temporal policy found; skipping training"
else
    echo "[$(date -Is)] starting WRM_temporal; batch_size=${batch_size}; num_workers=${num_workers}"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_temporal.yaml \
        --val-episodes 50-74 \
        --wandb-project "${wandb_project}" \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_temporal-cluster6-bs32" \
...
milestones = sorted(folder.glob("WRM_temporal_step_*.pt"))
if len(milestones) != 4 or not (folder / "last.pt").is_file():
    raise SystemExit(
        "Refusing incomplete WRM_temporal upload: "
...
api.upload_folder(
    ...
    commit_message="Upload WRM_temporal checkpoints through step 100000",
)
...
echo "[$(date -Is)] completed Cluster6 batch-size-32 WRM_temporal training"
