...
stage_dir="/tmp/airo_wrm_wrap_stratified_cluster9_stage"
source_archive="${stage_dir}/airo-doffy-wrm-wrap-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-wrap-src.sha256"
...
work_root="/tmp/airo_wrm_wrap_stratified_cluster9_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_wrap_stratified_cluster9_bs32"
policy_output="${output_root}/WRM_wrap"
policy_log="${policy_output}/cluster9_bs32_train.log"
wandb_project="AIRO-Doffy-WRM-Grasp"
batch_size=32
num_workers=8
upload_hf="${UPLOAD_HF:-0}"
hf_repo_id="${HF_REPO_ID:-IXDLI/AIRO-Doffy-WRM-Grasp-WRM-wrap}"
...
tests = sorted(Path("policies/realman_beaver/tests").glob("test_*wrap*.py"))
if not tests:
    raise SystemExit("No focused WRM_wrap unit-test module is present in the source archive")
print("wrap_test_modules", ",".join(str(path) for path in tests))
PY
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed WRM_wrap policy found; skipping training"
else
    echo "[$(date -Is)] starting WRM_wrap stratified; batch_size=${batch_size}; num_workers=${num_workers}"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_wrap.yaml \
        --wandb-project "${wandb_project}" \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_wrap-stratified-cluster9-bs32" \
...
milestones = sorted(folder.glob("WRM_wrap_step_*.pt"))
required = {
    folder / "WRM_wrap_step_025000.pt",
    folder / "WRM_wrap_step_050000.pt",
    folder / "WRM_wrap_step_075000.pt",
    folder / "WRM_wrap_step_100000.pt",
}
if not (folder / "last.pt").is_file() or any(not path.is_file() for path in required):
    raise SystemExit(
        "Refusing incomplete WRM_wrap upload: "
...
api.upload_folder(
    ...
    commit_message="Upload WRM_wrap stratified checkpoints through step 100000",
)
...
echo "[$(date -Is)] completed Cluster9 batch-size-32 WRM_wrap stratified training"
