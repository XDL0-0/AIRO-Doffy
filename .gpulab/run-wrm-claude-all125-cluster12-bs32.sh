...
# WRM_claude final all-125 run: no held-out episodes; every frame trains the
# frozen rollout checkpoint. All statistics are fit from the full 125-episode
# split, which is the final deployment population. Budget mirrors the
# architecture-selection run (100k steps, milestones every 10k).

set -Eeuo pipefail

stage_dir="/tmp/airo_wrm_claude_all125_cluster12_stage"
source_archive="${stage_dir}/airo-doffy-wrm-claude-all125-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-claude-all125-src.sha256"
...
work_root="/tmp/airo_wrm_claude_all125_cluster12_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_claude_all125_cluster12_bs32"
policy_output="${output_root}/WRM_claude_all125"
policy_log="${policy_output}/cluster12_all125_bs32_train.log"
wandb_project="AIRO-Doffy-WRM-Grasp"
batch_size=32
num_workers=8
upload_hf="${UPLOAD_HF:-0}"
hf_repo_id="${HF_REPO_ID:-IXDLI/AIRO-Doffy-WRM-Grasp-WRM-claude-all125}"
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed all-125 WRM_claude policy found; skipping"
else
    echo "[$(date -Is)] starting all-125 WRM_claude on cluster 12"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_claude_all_train.yaml \
        --wandb-project "${wandb_project}" \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_claude-all125-cluster12-bs32" \
...
api.upload_folder(
    ...
    commit_message="Upload WRM_claude all-125 milestones through step 100000",
)
...
echo "[$(date -Is)] completed Cluster12 batch-size-32 WRM_claude all-125 training"
