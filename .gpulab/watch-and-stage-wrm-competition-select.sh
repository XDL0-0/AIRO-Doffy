#!/usr/bin/env bash
# Stage source+dataset into a running GPUlab competition-selection job.
...
source_archive="/tmp/airo-doffy-wrm-competition-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-competition-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-competition-select.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_competition_select_stage"
...
if [[ ! -s "${source_archive}" || ! -s "${source_manifest}" ]]; then
    tar -C "${repo_root}" \
        --exclude='*/__pycache__' \
        --exclude='*.pyc' \
        -czf "${source_archive}" \
...
if [[ ! -s "${dataset_archive}" || ! -s "${dataset_manifest}" ]]; then
...
while true; do
    ...
    echo "[$(date -Is)] WRM competition ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
scp -o BatchMode=yes \
    "${wandb_netrc_file}" \
    "${ssh_alias}:${remote_stage}/netrc"
scp -o BatchMode=yes \
    "${hf_token_file}" \
    "${ssh_alias}:${remote_stage}/hf_token"

ssh -o BatchMode=yes "${ssh_alias}" \
    "chown -R 1000:100 ${remote_stage}; \
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-competition-select.sh; \
...
echo "Staged local source/dataset archives and released WRM competition job ${job_id}"
