...
source_archive="/tmp/airo-doffy-rdp-like-key4-src.tar.gz"
source_manifest="/tmp/airo-doffy-rdp-like-key4-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-rdp-like-key4-all125-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_rdp_like_key4_stage"
...
if [[ ! -f "${dataset_root}/meta/stats.json" ]]; then
    echo "Dataset is missing ${dataset_root}/meta/stats.json" >&2
    exit 2
fi
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 RDP-like Key4 ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-rdp-like-key4-all125-cluster6-bs32.sh; \
...
echo "Staged inputs and released Cluster6 RDP-like Key4 job ${job_id}"
