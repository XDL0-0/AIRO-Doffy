...
source_archive="/tmp/airo-doffy-wrm-delta-all125-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-delta-all125-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-delta-all125-cluster9-bs32.sh"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_delta_all125_cluster9_stage"

for required_file in "${gpulab_cli}" "${gpulab_cert}" "${runner_script}" "${wandb_netrc_file}"; do
    [[ -s "${required_file}" ]] || { echo "Missing local input: ${required_file}" >&2; exit 2; }
done
[[ -d "${dataset_root}" ]] || { echo "Missing dataset: ${dataset_root}" >&2; exit 2; }

tar -C "${repo_root}" --exclude='*/__pycache__' --exclude='*.pyc' \
    -czf "${source_archive}" policies/realman_beaver eval_config.py eval_policy.py
...
while true; do
    ...
    echo "[$(date -Is)] cluster9 all125 ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING) break ;;
...
scp -o BatchMode=yes "${source_archive}" "${source_manifest}" \
    "${dataset_archive}" "${dataset_manifest}" "${runner_script}" \
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} ${remote_stage}/run-wrm-delta-all125-cluster9-bs32.sh; \
...
echo "Staged and released all-125 cluster-9 job ${job_id}"
