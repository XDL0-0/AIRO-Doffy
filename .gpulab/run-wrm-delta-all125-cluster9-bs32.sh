...
stage_dir="/tmp/airo_wrm_delta_all125_cluster9_stage"
source_archive="${stage_dir}/airo-doffy-wrm-delta-all125-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-delta-all125-src.sha256"
...
work_root="/tmp/airo_wrm_delta_all125_cluster9_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_delta_all125_cluster9_bs32"
policy_output="${output_root}/WRM_delta_all_train"
policy_log="${policy_output}/cluster9_all125_bs32_train.log"
...
for required_file in \
    ...
    "${dataset_manifest}" \
    "${wandb_netrc_file}"; do
    [[ -s "${required_file}" ]] || { echo "Missing staged input: ${required_file}" >&2; exit 2; }
done
...
print("gpu", torch.cuda.get_device_name(0), "probe", tuple((probe @ probe).shape))
print("resources", "visible_cpus", len(os.sched_getaffinity(0)), "batch_size", 32)
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed all-125 WRM_delta policy found; skipping"
else
    echo "[$(date -Is)] starting all-125 WRM_delta on cluster 9"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_delta_all_train.yaml \
        --wandb-project AIRO-Doffy-WRM-Grasp \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_delta-all125-cluster9-bs32" \
...
echo "[$(date -Is)] completed all-125 WRM_delta training"
