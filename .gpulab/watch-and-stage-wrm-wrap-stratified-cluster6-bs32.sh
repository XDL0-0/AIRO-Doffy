...
runner_script="${repo_root}/.gpulab/run-wrm-wrap-stratified-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_wrap_stratified_cluster6_stage"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 WRM_wrap ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-wrap-stratified-cluster6-bs32.sh; \
...
echo "Staged local source/dataset archives and released Cluster6 WRM_wrap job ${job_id}"
