...
source_archive="/tmp/airo-doffy-wrm-wrap-delta-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-wrap-delta-src.sha256"
dataset_archive="/tmp/WRM_grasp_cylinder_different_sizes_lero_tightness-wrap-delta.tar"
dataset_manifest="/tmp/WRM_grasp_cylinder_different_sizes_lero_tightness-wrap-delta.sha256"
runner_script="${repo_root}/.gpulab/run-wrm-wrap-delta-stratified-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_wrap_delta_cluster6_stage"
...
echo "Prepared source archive: $(cat "${source_manifest}")"
echo "Prepared local dataset archive: $(cat "${dataset_manifest}")"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 WRM_wrap_delta ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
for ssh_attempt in $(seq 1 20); do
    if ssh \
        ...
        -o ConnectTimeout=20 \
        "${ssh_alias}" \
        "mkdir -p ${remote_stage} && chmod 755 ${remote_stage}"; then
        break
    fi
...
scp -o BatchMode=yes "${hf_token_file}" "${ssh_alias}:${remote_stage}/hf_token"

ssh -o BatchMode=yes "${ssh_alias}" \
    "chown -R 1000:100 ${remote_stage}; \
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-wrap-delta-stratified-cluster6-bs32.sh; \
    chmod 600 ${remote_stage}/netrc; \
    chmod 600 ${remote_stage}/hf_token; \
    cd ${remote_stage}; \
...
echo "Staged local source/dataset archives and released Cluster6 WRM_wrap_delta job ${job_id}"
