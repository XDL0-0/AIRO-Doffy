...
repo_root="/home/yuyuan/AIRO-Doffy-agent-qwen"
dataset_root="${repo_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
source_archive="/tmp/airo-doffy-wrm-qwen-all125-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-qwen-all125-src.sha256"
...
runner_script="${repo_root}/.gpulab/run-wrm-qwen-all125-cluster6-bs32.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
wandb_netrc_file="/home/yuyuan/.netrc"
remote_stage="/tmp/airo_wrm_qwen_all125_cluster6_stage"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster6 WRM_qwen-all125 ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} \
        ${remote_stage}/run-wrm-qwen-all125-cluster6-bs32.sh; \
...
echo "Staged local source/dataset archives and released Cluster6 WRM_qwen-all125 job ${job_id}"
