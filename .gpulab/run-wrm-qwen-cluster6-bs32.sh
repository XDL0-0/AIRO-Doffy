...
stage_dir="/tmp/airo_wrm_qwen_cluster6_stage"
source_archive="${stage_dir}/airo-doffy-wrm-qwen-src.tar.gz"
source_manifest="${stage_dir}/airo-doffy-wrm-qwen-src.sha256"
...
work_root="/tmp/airo_wrm_qwen_cluster6_work"
repo_root="${work_root}/AIRO-Doffy"
dataset_root="${work_root}/datasets/WRM_grasp_cylinder_different_sizes_lero_tightness"
output_root="/project_ghent/AIRO-Doffy/WRM_grasp_cylinder_different_sizes_lero_tightness/wrm_qwen_cluster6_bs32"
policy_output="${output_root}/WRM_qwen"
policy_log="${policy_output}/cluster6_bs32_train.log"
wandb_project="AIRO-Doffy-WRM-Grasp"
batch_size=32
num_workers=8
upload_hf="${UPLOAD_HF:-0}"
hf_repo_id="${HF_REPO_ID:-IXDLI/AIRO-Doffy-WRM-Grasp-WRM-qwen}"
...
if [[ -f "${policy_output}/last.pt" ]]; then
    echo "[$(date -Is)] existing completed WRM_qwen policy found; skipping training"
else
    echo "[$(date -Is)] starting WRM_qwen architecture-selection run; batch_size=${batch_size}; num_workers=${num_workers}"
    set -o pipefail
    "${python_bin}" -m policies.realman_beaver.train \
        --config policies/realman_beaver/configs/WRM_qwen.yaml \
        --val-episodes 20-24,45-49,70-74,95-99,120-124 \
        --wandb-project "${wandb_project}" \
        --wandb-run-name "WRM_grasp_cylinder_different_sizes_lero_tightness/WRM_qwen-cluster6-bs32" \
...
if len(milestones) != 10 or not (folder / "last.pt").is_file():
    raise SystemExit(
        "Refusing incomplete WRM_qwen upload: "
...
api.upload_folder(
    ...
    commit_message="Upload WRM_qwen checkpoints through step 100000",
)
...
echo "[$(date -Is)] completed Cluster6 batch-size-32 WRM_qwen architecture-selection training"
