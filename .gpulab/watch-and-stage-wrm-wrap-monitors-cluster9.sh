...
source_archive="/tmp/airo-doffy-wrm-wrap-monitors-src.tar.gz"
source_manifest="/tmp/airo-doffy-wrm-wrap-monitors-src.sha256"
dataset_archive="/tmp/WRM_grasp_cylinder_different_sizes_lero_tightness-monitors.tar"
dataset_manifest="/tmp/WRM_grasp_cylinder_different_sizes_lero_tightness-monitors.sha256"
runner_script="${repo_root}/.gpulab/run-wrm-wrap-monitors-cluster9.sh"
hf_token_file="/home/yuyuan/.cache/huggingface/token"
remote_stage="/tmp/airo_wrm_wrap_monitors_stage"
...
if [[ ! -d "${dataset_root}" ]]; then
    echo "Missing requested dataset: ${dataset_root}" >&2
    exit 2
fi
...
echo "Prepared dataset archive: $(cat "${dataset_manifest}")"
...
while true; do
    ...
    echo "[$(date -Is)] Cluster9 monitors ${short_id} status=${status:-unknown}"
    case "${status}" in
        RUNNING)
            break
            ;;
...
ssh -o BatchMode=yes "${ssh_alias}" \
    ...
    chmod 755 ${remote_stage} ${remote_stage}/run-wrm-wrap-monitors-cluster9.sh; \
...
echo "Staged source/dataset and released monitor job ${job_id}"
