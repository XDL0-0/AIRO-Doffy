"""Typed configuration for three LeRobot-backed Realman policies."""
...
SUPPORTED_VARIANTS = {"original_dp", "dp_beaver", "rdp_like"}
...
class ModelConfig:
    ...
    # Canonical LeRobot Diffusion Policy settings.
    n_obs_steps: int = 2
    horizon: int = 16
...
class RDPConfig:
    ...
    # slow LDP samples a fresh one. 8 ticks at 24 Hz ~= 3 Hz, matching the
    # DP variants' n_action_steps=8 receding horizon. At downsample 2, one
    # replan consumes tokens 0-3 of the 16-token chunk.
    slow_replan_steps: int = 8

    # LeRobot Diffusion Policy settings for latent action sequences.
    latent_down_dims: tuple[int, ...] = (256, 512, 1024)
    latent_kernel_size: int = 3
    # RDP reference: full 100-step DDPM sampling (no DDIM shortcut).
    latent_num_inference_steps: int = 100
    latent_noise_scheduler_type: str = "DDPM"

    # Reference RDP trains the AT for 601 epochs and the LDP for 401 epochs,
    ...
    # Resume the latent-DP stage from a "latent_dp" checkpoint and keep
...
class RealmanBeaverConfig:
    ...
    def from_dict(cls, raw: dict[str, Any]) -> RealmanBeaverConfig:
        allowed = {"dataset", "model", "rdp", "training"}
    ...
    def validate(self) -> None:
        ...
        if model.horizon % (2 ** len(model.down_dims)):
            raise ValueError(
                "DP horizon must be divisible by the LeRobot U-Net downsampling factor"
            )
        ...
        if not 1 <= model.num_inference_steps <= model.num_train_timesteps:
            raise ValueError(
                "num_inference_steps must be within the training diffusion schedule"
            )
        if model.variant == "rdp_like":
            if rdp.action_horizon % rdp.downsample_ratio:
                raise ValueError(
                    "RDP action_horizon must be divisible by downsample_ratio"
                )
            if rdp.downsample_ratio <= 0 or rdp.downsample_ratio & (
                rdp.downsample_ratio - 1
            ):
                raise ValueError("RDP downsample_ratio must be a positive power of two")
            if rdp.latent_horizon % (2 ** len(rdp.latent_down_dims)):
                raise ValueError(
                    "latent horizon must be divisible by the latent U-Net downsampling factor"
                )
            if not 1 <= rdp.slow_replan_steps <= rdp.action_horizon:
                raise ValueError("slow_replan_steps must be in [1, action_horizon]")
            if rdp.kl_weight < 0 or rdp.latent_dim <= 0:
                raise ValueError(
                    "kl_weight must be non-negative and latent_dim positive"
                )
