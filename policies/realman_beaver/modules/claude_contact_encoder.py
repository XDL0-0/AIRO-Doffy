"""Cell-token contact-field encoder for the WRM_claude policy.

WRM_claude consumes a short causal Beaver history from the four reliable
near-field sensors (01/02/10/11) as a fine-grained *cell-token* contact field:
every validity-masked 4x4 cell becomes one token, so the encoder sees which
part of the enclosure is touching rather than only which sensor fired. This
matters for whole-arm grasping of different bottle sizes, where the same
sensor can be triggered by very different contact geometry.

The representation is deliberately size-agnostic: normalized absolute range,
within-sensor and global relative geometry, multiple physical proximity
scales, and changes over several temporal baselines. Invalid and zero cells
contribute strictly zero geometry while their missingness remains visible.
A cross-sensor Transformer lets cells exchange information, and quality-
weighted attention pooling produces the 64-D conditioning feature. An
all-invalid frame yields an exactly zero feature.
"""
...
class ContactFieldEncoder(nn.Module):
    """Encode a validity-masked temporal Beaver contact field as cell tokens."""
    ...
    def __init__(
        ...
        *,
        ...
        if not indices or len(set(indices)) != len(indices):
            raise ValueError("Contact-field sensor indices must be non-empty/unique")
        if any(index < 0 or index >= n_sensors for index in indices):
            raise ValueError("Contact-field sensor index is out of range")
        if not statuses:
        ...
        self.cell_mlp = nn.Sequential(
            nn.Linear(self.cell_feature_dim, sensor_hidden_dim),
            nn.SiLU(),
            nn.Linear(sensor_hidden_dim, token_dim),
            nn.LayerNorm(token_dim),
            nn.SiLU(),
        )
        # One learned embedding per physical sensor keeps cell identities
        # distinguishable while the shared MLP stays size-agnostic.
        ...
        self.cell_transformer = nn.TransformerEncoder(
    ...
    def set_normalization_statistics(
        ...
        for name, value in {"p5": p5, "p95": p95, "median": median}.items():
            ...
            if tensor.shape != (self.n_selected_sensors,):
                raise ValueError(
                    f"Contact-field {name} must have shape "
                    f"({self.n_selected_sensors},)"
                )
            if not torch.isfinite(tensor).all():
                raise ValueError(f"Contact-field {name} must be finite")
            converted[name] = tensor
        if torch.any(converted["p95"] <= converted["p5"]):
            raise ValueError("Contact-field p95 must exceed p5 per sensor")
    ...
    def _select(
        ...
        if distance.ndim < 4 or distance.shape[-2:] != (4, 4):
            raise ValueError(
                "Contact-field distance must end in [history, sensor, 4, 4]"
            )
        ...
        if status.shape != distance.shape or present.shape != distance.shape[:-2]:
            raise ValueError("Contact-field distance/status/present shapes differ")
        ...
        raise ValueError("Contact-field inputs do not contain configured sensors")
    ...
    def preprocess(
        ...
        """Return per-cell features ``(..., 4, 16, cell_feature_dim)``."""
        if not bool(self.normalization_fitted.item()):
            raise RuntimeError(
                "Contact-field normalization statistics are not fitted; "
        ...
        # Invalid cells must not inject the per-sensor median into the policy.
        # Their geometry is strictly zero; the validity and raw-zero channels
        # below retain the missingness information.
        ...
        return cell_features, {
            ...
            "proximity": proximity,
            "cell_available": current_valid.flatten(start_dim=-2),
            # A cell with a sporadically valid history should not compete on
            # equal footing with a spatially and temporally coherent field.
            "cell_quality": torch.sqrt(
                current_float
                * genuine.to(distance.dtype).mean(dim=-4)
            ).flatten(start_dim=-2),
        }
    ...
    def forward(
        ...
        *,
        ...
        cells = self.n_selected_sensors * 16
        # Merge the two 4x4 spatial dims into 16 cell tokens, keeping the
        # sensor dim and the trailing feature vector intact.
        cell_vectors = cell_features.flatten(start_dim=-3, end_dim=-2)
        cell_tokens = self.cell_mlp(cell_vectors)
        # One embedding per sensor broadcasts across its 16 cells; the view
        # must keep exactly cell_tokens' rank (a larger rank would silently
        # prepend a singleton leading dimension).
        embedding = self.sensor_embedding.unsqueeze(-2)
        embedding_shape = [1] * (cell_tokens.ndim - 3) + [
            self.n_selected_sensors,
            1,
            -1,
        ]
        cell_tokens = cell_tokens + embedding.view(*embedding_shape)

        leading_shape = cell_tokens.shape[:-3]
        flat_tokens = cell_tokens.reshape(-1, cells, cell_tokens.shape[-1])
        available = intermediates["cell_available"].reshape(-1, cells)
        ...
        quality = intermediates["cell_quality"].reshape(-1, cells)
            ...
            intermediates.update(
                {
                    ...
                    "encoded_cell_tokens": encoded.reshape(
                        *leading_shape, cells, encoded.shape[-1]
                    ),
                    "cell_attention": attention.reshape(*leading_shape, cells),
                }
            )
