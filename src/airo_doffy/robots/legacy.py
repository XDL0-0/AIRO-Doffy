"""Legacy robot adapters retained behind the root compatibility facade.
...
class RobotBackend:
    ...
    def is_joint_target_safe(
        ...
        if previous_joints is not None and not utils.is_joint_change_safe(
            previous_joints,
            joints,
            joint_threshold,
        ):
...
class PositionManipulatorBackend(RobotBackend):
    ...
    def get_tcp_force(self) -> np.ndarray | None:
        ...
        if realman_robot is not None and hasattr(realman_robot, "rm_get_force_data"):
            ...
            if error_code != 0:
                utils.logger.warning(
                    f"RealMan rm_get_force_data failed with error code {error_code}."
                )
                return None
...
class URPositionBackend(PositionManipulatorBackend):
    ...
    def is_joint_target_safe(
        ...
        if not utils.is_joint_within_limits(joints):
            utils.logger.warning(
                "No valid IK solution within UR joint limits, keeping previous pose!"
            )
            return False
...
class URTorqueBackend(RobotBackend):
    ...
    def start_freedrive(self) -> None:
        utils.logger.warning(
            "Freedrive requested in torque mode. Trying teachMode without disabling "
            "torque control; "
